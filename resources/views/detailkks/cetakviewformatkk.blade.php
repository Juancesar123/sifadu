<table style="width:80%" cellpadding="2" cellspacing="0">
        <tbody>
            <tr>
                <td style="text-align: left">
                    <img style="width: 110px;height: auto" src="{{asset('img/2191f-lambang_kab_sukabumi.svg_-2-e1532000502726-232x300.png')}}" alt="">
                </td>
                <td style="text-align: center">
                    <div style="font-size: 20px;font-weight: bold">
                       Pemerintah Kabupaten Sukabumi
                       <p style="font-size:30px;font-weight:bold">
                            test development
                       </p>
                       <P style="font-size:12px;font-weight:normal">
                           Jalan Raya Rambai No. 70 Cisaat Telp.(0266) 211075 sukabumi
                       </P>

                    </div>
                </td>
            </tr>
            <tr>
               <td colspan="2" >
                   <hr style="border-style:double;border-width: 2px; width:122%">
                </td>
            </tr>
            {{-- <tr style="padding-left: 30px;">
                <td style="text-align: center" colspan="2">
                    <div style="font-size: 20px;font-weight:bold">
                        Keterangan Pindah Datang WNI Dalam Kab/Kota
                    </div>
                </td>
            </tr> --}}
            <tr style="padding-left: 30px;">
                    <td style="text-align: left;padding-bottom: 10px;">
                        <div style="font-size: 15px">
                            DATA KEPALA KELUARGA
                        </div>
                    </td>
                    {{-- <td style="
                    text-align: right;padding-bottom: 20px ;
                    width: 100%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                          Kode Nama provinsi
                        </div>
                    </td> --}}
                </tr>

            <tr style="padding-left: 30px;">
                    <td style="text-align: center;padding-bottom: 10px;">
                            <div style="font-size: 15px">
                                NAMA KEPALA KELUARGA
                            </div>
                        </td>
                <td  style="
                text-align: left;
                padding-bottom: 10px;
                width: 90%;
                overflow: hidden;
                display: inline-block;
                white-space: nowrap;
                ">
                    <div style="width:90%; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                </td>
                <tr style="padding-left: 20px; height:10px">
                        <td style="text-align: center;padding-bottom: 10px;">
                            <div style="font-size: 15px">
                                ALAMAT &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            </div>
                        </td>
                        <td  style="
                        text-align: left;
                        padding-bottom: 10px;
                        width: 90%;
                        overflow: hidden;
                        display: inline-block;
                        white-space: nowrap;
                        ">
                            <div style="width:90%; border-width: 1px; border-style:solid; height:30px;float:left">

                            </div>
                        </td>
                    </tr>
                    <tr style="padding-left: 20px; height:10px">
                            <td style="text-align: center;padding-bottom: 10px;">
                                    <div style="font-size: 15px">
                                        Kode Pos &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    </div>
                                </td>
                            <td  style="
                            text-align: left;
                            padding-bottom: 10px;
                            width:90%;
                            overflow: hidden;
                            display: inline-block;
                            white-space: nowrap;
                            ">
                                <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px;float:left;height:30px;">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:166px;float:left;height:30px;">
                                        &nbsp;Jumlah Anggota keluarga
                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                    </div>
                            </td>
                        </tr>
                        <tr style="padding-left: 20px; height:10px">
                                <td style="text-align: center;padding-bottom: 10px;">
                                    <div style="font-size: 15px">
                                        Telepon &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    </div>
                                </td>
                                <td  style="
                                text-align: left;
                                padding-bottom: 10px;
                                width:90%;
                                overflow: hidden;
                                display: inline-block;
                                white-space: nowrap;
                                ">
                                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>

                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                                        </div>
                                </td>
                            </tr>
                    {{-- <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div> --}}
                </td>
            </tr>
        </tbody>
    </table>

            {{-- <tr style="padding-left: 30px; height:10px">
                <td style="text-align: left;padding-bottom: 20px;">
                    <div style="font-size: 15px">
                        2. Nama Kepala Keluarga:
                    </div>
                </td>
                <td>
                    <div style="width:80%; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                </td>
            </tr>
            <tr style="padding-left: 30px; height:10px">
                <td style="
                text-align: left;
                padding-bottom: 20px;
                 ">
                    <div style="font-size: 15px">
                        3. Alamat:
                    </div>

                </td>
                <td  style="
                text-align: left;
                padding-bottom: 20px;
                width: 40%;
                overflow: hidden;
                display: inline-block;
                white-space: nowrap;
                ">
                    <div style="width:90%; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                </td>
                <td style="
                text-align: left;padding-bottom: 20px ;
                width: 4%;
                overflow: hidden;
                display: inline-block;
                white-space: nowrap;">
                    <div style="font-size: 15px">
                        Rt:
                    </div>
                </td>
                <td style="
                text-align: left;padding-bottom: 20px;
                width: 20%;
                overflow: hidden;
                display: inline-block;
                white-space: nowrap;
                ">
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                </td>
                <td style="
                text-align: left;padding-bottom: 20px ;
                width: 4%;
                overflow: hidden;
                display: inline-block;
                white-space: nowrap;">
                    <div style="font-size: 15px">
                        Rw:
                    </div>
                </td>
                <td style="
                text-align: left;padding-bottom: 20px;
                width: 20%;
                overflow: hidden;
                display: inline-block;
                white-space: nowrap;
                ">
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                    <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                    </div>
                </td>
            </tr>
            <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-left: 80px;
                    padding-bottom:20px;
                     ">
                        <div style="font-size: 15px">
                            a. Desa/Kelurahan:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 50%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:90%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 15%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                          c.  Kab/Kota:
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:100%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-left: 80px;
                    padding-bottom:20px;
                        ">
                        <div style="font-size: 15px">
                            b. Kecamatan:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 50%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:90%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 15%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            d.  Provinsi:
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:100%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-left: 80px;
                    padding-bottom:20px;
                        ">
                        <div style="font-size: 15px">
                            e. Kode Pos:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            f.  Nomor Telepon:
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 40%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px;">
                    <td style="text-align: left" colspan="2">
                        <div style="font-size: 15px; font-weight:bold">
                            Data Kepindahan:
                        </div>
                        <br>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                        <div style="font-size: 15px">
                            1. Alasan Pindah:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size:15px;font-weight:bold;width:10px; border-width: 1px; border-style:solid; height:30px;float:left">
                            5
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            1. Pekerjaan
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            3. Keamanan
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            5. Perumahan
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            7. Lainnya(Sebutkan)
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">

                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            2. Pendidikan
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            4. Kesehatan
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            6. Keluarga
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                        <div style="font-size: 15px">
                            2. Alamat Tujuan Pindah:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 40%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:90%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 4%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            Rt:
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 4%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            Rw:
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-left: 80px;
                    padding-bottom:20px;
                        ">
                        <div style="font-size: 15px">
                            a. Desa/Kelurahan:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 50%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:90%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 15%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            c.  Kab/Kota:
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:100%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-left: 80px;
                    padding-bottom:20px;
                        ">
                        <div style="font-size: 15px">
                            b. Kecamatan:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 50%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:90%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 15%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            d.  Provinsi:
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:100%; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-left: 80px;
                    padding-bottom:20px;
                        ">
                        <div style="font-size: 15px">
                            e. Kode Pos:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            f.  Nomor Telepon:
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 40%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                        <div style="width:30px; border-width: 1px; border-style:solid; height:30px;float:left">

                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                        <div style="font-size: 15px">
                            3. Klasifikasi Pindah:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size:15px;font-weight:bold;width:10px; border-width: 1px; border-style:solid; height:30px;float:left">
                            3
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 35%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            1. Dalam satu desa/kelurahan
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 25%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            3. Antar Kecamatan
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            5. Antar Provinsi
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">

                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 35%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            2. Antar Desa / Kelurahan
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 25%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            4. Antar Kab/Kota
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                        <div style="font-size: 15px">
                            4. Jenis Kepindahan:
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size:15px;font-weight:bold;width:10px; border-width: 1px; border-style:solid; height:30px;float:left">
                            2
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 55%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            1. Kep. Keluarga
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 35%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            3. Kep. Klrga dan Sbg Angg Klrga
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">

                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 55%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            2. Kep. Keluarga dan Seluruh Angg Keluarga
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 25%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            4. Angg Keluarga
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px" cellpadding="0">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                        <div style="font-size: 15px">
                            5. Status No KK bagi yang tidak pindah :
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size:15px;font-weight:bold;width:10px; border-width: 1px; border-style:solid; height:30px;float:left">
                            3
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            1. Numpang KK
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 50%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            3. Tidak ada Angg. Keluarga yang ditinggal
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">

                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 30%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            2. Membuat KK baru
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 25%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            4. Angg Keluarga
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px" cellpadding="0">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                        <div style="font-size: 15px">
                            6. Status No KK bagi yang pindah :
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 5%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size:15px;font-weight:bold;width:10px; border-width: 1px; border-style:solid; height:30px;float:left">
                            1
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size: 15px">
                            1. Numpang KK
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 25%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                            2. Membuat KK Baru
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 45%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size: 15px">
                           3  Nama Kep. Kelrg dan No. KK Ttp
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px" cellpadding="0">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                        <div style="font-size: 15px">
                            7. Rencana Tgl pindah :
                        </div>

                    </td>
                    <td  style="
                    text-align: left;
                    padding-bottom: 20px;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size:15px;font-weight:bold;text-align:center;width:30px; border-width: 1px; border-style:solid; height:30px;float:left">
                            2
                        </div>
                        <div style="font-size:15px;font-weight:bold;text-align:center;width:30px; border-width: 1px; border-style:solid; height:30px;float:left">
                            5
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px ;
                    width: 20%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;">
                        <div style="font-size:15px;font-weight:bold;text-align:center;width:30px; border-width: 1px; border-style:solid; height:30px;float:left">
                            0
                        </div>
                        <div style="font-size:15px;font-weight:bold;text-align:center;width:30px; border-width: 1px; border-style:solid; height:30px;float:left">
                            4
                        </div>
                    </td>
                    <td style="
                    text-align: left;padding-bottom: 20px;
                    width: 25%;
                    overflow: hidden;
                    display: inline-block;
                    white-space: nowrap;
                    ">
                        <div style="font-size:15px;font-weight:bold;text-align:center;width:30px; border-width: 1px; border-style:solid; height:30px;float:left">
                            1
                        </div>
                        <div style="font-size:15px;font-weight:bold;text-align:center;width:30px; border-width: 1px; border-style:solid; height:30px;float:left">
                            8
                        </div>
                    </td>
                </tr>
                <tr style="padding-left: 30px; height:10px" cellpadding="0">
                    <td style="
                    text-align: left;
                    padding-bottom: 20px;
                        ">
                        <div style="font-size: 15px">
                            8. Keluarga yang pindah :
                        </div>

                    </td>
                </tr>
                <tr>
                <table border="1" style="width:100%" cellpadding="2" cellspacing="0">
                    <thead>
                        <th>No</th>
                        <th colspan="16">Nik</th>
                        <th>Nama</th>
                        <th colspan="2">SHDK</th>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="width: 4%">
                               1
                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td style="width:30%">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">
                            </td>
                        </tr>
                        <tr>
                            <td style="width: 4%">
                                2
                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td style="width:30%">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">
                            </td>
                        </tr>
                        <tr>
                            <td style="width: 4%">
                                3
                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">

                            </td>
                            <td style="width:30%">

                            </td>
                            <td width="30px">

                            </td>
                            <td width="30px">
                            </td>
                        </tr>
                            <tr>
                                <td style="width: 4%">
                                    4
                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td style="width:30%">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">
                                </td>
                            </tr>
                            <tr>
                                <td style="width: 4%">
                                    5
                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td style="width:30%">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">
                                </td>
                            </tr>
                            <tr>
                                <td style="width: 4%">
                                    6
                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td style="width:30%">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">
                                </td>
                            </tr>
                            <tr>
                                <td style="width: 4%">
                                    7
                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td style="width:30%">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">
                                </td>
                            </tr>
                            <tr>
                                <td style="width: 4%">
                                    8
                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td style="width:30%">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">
                                </td>
                            </tr>
                            <tr>
                                <td style="width: 4%">
                                    9
                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td style="width:30%">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">
                                </td>
                            </tr>
                            <tr>
                                <td style="width: 4%">
                                    10
                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">

                                </td>
                                <td style="width:30%">

                                </td>
                                <td width="30px">

                                </td>
                                <td width="30px">
                                </td>
                            </tr>
                    </tbody>
                </table>
                </tr>

        </tbody>
    </table>
    <br>
    <div style="
    text-align: left;padding-bottom: 20px ;
    width: 25%;
    float:left;">
        Diketahui Oleh:
        <p>Kepala Dinas Dafduk</p>
        <p>ub. Kasi.................</p>
        <p>No. .............. tgl .........20......</p>
        <br>
        <br>
        (..........................)

        <p>Nip</p>
    </div>
    <div style="
    text-align: left;padding-bottom: 20px ;
    width: 25%;
    float:left">
        Diketahui Oleh:
        <p>Camat ..........</p>
        <p>No. .............. tgl .........20......</p>
        <br>
        <br>
        <br>
        <br>
        (..........................)
        <p>Nip</p>
    </div>
    <div style="
    text-align: left;padding-bottom: 20px ;
    width: 25%;
    float:left">
        Pemohon
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        (..........................)
        <p>Nip</p>
    </div>
    <div style="
    text-align: left;padding-bottom: 20px ;
    width: 25%;
    float:left">
        Dikeluarkan Oleh:
        <p>Kepala Desa {{ env('DESA_NAME') }}</p>
        <p>No.475/...../Tgl.06 Agustus 2018</p>
        <br>
        <br>
        <br>

        (..........................)
        <p>Nip</p>
    </div> --}}